...
SCORE_THRESHOLD = 0.28  # снижено с 0.30 — реальные прогоны показали кандидатов
# на 0.2965/0.2998, впритык под порогом, которые уходили в дорогой
# retry-проход (generate_retry_queries + повторный archive_pool, сотни
# секунд) почти без шанса найти что-то лучше. Мы не ищем 100% совпадение —
# для документалки/публицистики "похоже на архивное" достаточно.
...
# даёт текст, а CLIP даёт число, которое сравнивается с зашитым SCORE_THRESHOLD
# выше. У более крупных моделей (например, ViT-L-14) другое распределение
# косинусных сходств — тот же порог 0.24 на них не откалиброван, и молчаливая
# подмена модели по device могла бы незаметно испортить решения "matched" vs
# "low_confidence" вместо того, чтобы просто ускорить то же самое поведение.
# GPU (Colab/T4) уже даёт ускорение и на этой модели — см. ClipScorer.device.
# Переключение на более крупную модель — сознательное решение, не дефолт:
# передайте ClipScorer(model_name="ViT-L-14", pretrained="openai") явно и
# сначала прогоните на паре реальных проектов, сравнив score у одних и тех
# же кандидатов с текущей SCORE_THRESHOLD, прежде чем полагаться на неё
# в проде — как и с любой другой моделью, которую здесь заменит.
