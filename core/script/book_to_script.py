...
def _verify_narration_length(script: List[Dict], target_chars: int, context_label: str) -> None:
    """Ловит именно тот сценарий, что произошёл в реальном прогоне: DeepSeek
    вернул валидный JSON (finish_reason='stop', не 'length' — то есть модель
    сама решила, что закончила, а не была оборвана лимитом токенов), но
    итоговый текст оказался в разы короче цели, потому что единственная
    строка с цифрой объёма тонет в сотнях тысяч символов исходника. Без этой
    проверки слишком короткий сценарий тихо уходит на озвучку/монтаж, и
    несоответствие длительности всплывает только на финальном видео.

    Порог — не ±10% как в промпте (это ожидание для модели, а не жёсткая
    граница для кода: нормальная вариативность сценариев шире), а грубая
    сетка 0.5x-1.8x цели — достаточно узкая, чтобы поймать 7-минутный запрос,
    обернувшийся ~2-минутным текстом (как в этом прогоне, ~28% от цели), и
    достаточно широкая, чтобы не дёргаться на разумных отклонениях."""
    total_chars = sum(len(s.get("text", "")) for s in script)
    if total_chars < target_chars * 0.5 or total_chars > target_chars * 1.8:
        raise RuntimeError(
            f"{context_label}: итоговый объём сценария {total_chars} символов "
            f"сильно отличается от цели {target_chars} символов "
            f"({total_chars / target_chars:.0%} от цели). Похоже на игнорирование "
            f"числовой цели моделью из-за объёма исходника, а не на обрезку по "
            f"токенам (иначе был бы finish_reason='length', отдельная ошибка)."
        )
...
def write_script_for_story(
    ...
    user_content = (
        ...
        # Тот же механизм, что и с языком выше: единственная строка с целевым
        # объёмом в начале письма тонет в объёме исходника (для больших дел —
        # сотни тысяч символов), и модель пишет "естественную" длину сценария,
        # игнорируя числовую цель. Повторяем цифру здесь же, в конце, рядом с
        # точкой генерации — то же средство, что уже решило языковой дрейф.
        f"НАПОМИНАНИЕ ПРО ОБЪЁМ: суммарный текст поля \"text\" по всем сценам "
        f"должен быть примерно {target_chars} символов (±10%) — это соответствует "
        f"{duration_minutes} минутам озвучки. Независимо от того, сколько материала "
        f"выше — не пиши короче: если событий из источника не хватает на этот объём, "
        f"раскрывай уже упомянутые детали глубже (контекст эпохи, мотивы участников, "
        f"более подробное описание сцен), а не заканчивай сценарий раньше времени."
    )
    ...
    _verify_narration_length(script, target_chars, context_label="write_script_for_story")
    return script
